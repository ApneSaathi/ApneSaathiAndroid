...
class CircleImageView : ImageView {
    ...
    @Deprecated(
        ...
        ReplaceWith("circleBackgroundColor = ContextCompat.getColor(context, circleBackgroundRes)")
    )
    ...
    private fun initializeBitmap() {
        ...
        mBitmapCanvas = if (mBitmap != null && mBitmap!!.isMutable) Canvas(mBitmap!!) else null

        if (!mInitialized) return

        if (mBitmap != null) updateShaderMatrix()
        else mBitmapPaint.shader = null
    }
    ...
    private fun inTouchableArea(x: Float, y: Float): Boolean {
        return if (mBorderRect.isEmpty) {
            true
        } else Math.pow(x - mBorderRect.centerX().toDouble(), 2.0) +
                Math.pow(y - mBorderRect.centerY().toDouble(), 2.0) <=
